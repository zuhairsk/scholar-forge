...
export function Sidebar() {
  ...
  return (
    <motion.aside 
      ...
      className="z-40 h-screen bg-card border-r border-border flex flex-col transition-all duration-300 relative flex-shrink-0"
    >
      ...
      <div className="px-6 py-4 flex items-center gap-3 border-b border-border/50">
        <div className="w-10 h-10 flex-shrink-0 rounded-full bg-secondary border border-primary/30 flex items-center justify-center text-primary font-serif">
          {CURRENT_USER.avatar}
        </div>
        {sidebarOpen && (
        ...
        {links.map((link) => {
          ...
          return (
            <Link key={link.href} href={link.href} className={cn(
              "flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors group relative whitespace-nowrap",
              isActive ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-white/5 hover:text-foreground"
            )}>
              <link.icon size={20} className={cn("flex-shrink-0", isActive && "drop-shadow-[0_0_8px_rgba(212,168,67,0.5)]")} />
      ...
      {sidebarOpen && (
        <div className="p-4 m-4 rounded-xl bg-gradient-to-br from-primary/20 to-transparent border border-primary/20">
          ...
          <p className="text-xs text-muted-foreground mb-3 leading-tight">Get detailed AI review insights.</p>
